...
def parse_draft_board():
    ...
    draft_sections = [
        {'indices': (0, 1), 'filename': './results/al_draft.txt'},  # Columns A & B
        {'indices': (2, 3), 'filename': './results/cl_draft.txt'},  # Columns C & D
        {'indices': (4, 5), 'filename': './results/ml_draft.txt'},  # Columns E & F
    ]
    # -----------------------------
    ...
    for section in draft_sections:
        ...
        with open(output_file, 'w') as f:
            for index, row in df.iterrows():
                ...
                if pick_row_number % picks_per_round == 1 and round <= 5:
